...
public class RDBKnownUriFilterTest {
    ...
    public void testGetOutdatedUris() throws URISyntaxException, UnknownHostException {
        ...
        Cursor<Boolean> cursor = r.db(DATABASE_NAME).table(TABLE_NAME).
            filter(doc -> doc.getField(COLUMN_URI).eq(uri1.getUri().toString())).
            getField(COLUMN_CRAWLING_IN_PROCESS).run(connector.connection);

        // check if flag is true for uri1
        Assert.assertTrue(cursor.next());

        cursor = r.db(DATABASE_NAME).table(TABLE_NAME).
            filter(doc -> doc.getField(COLUMN_URI).eq(uri2.getUri().toString())).
            getField(COLUMN_CRAWLING_IN_PROCESS).run(connector.connection);
        ...
        r.db(DATABASE_NAME).table(TABLE_NAME).
            filter(doc -> doc.getField(COLUMN_URI).eq(uri1.getUri().toString())).
            update(r.hashMap(COLUMN_TIMESTAMP_LAST_CRAWL,
